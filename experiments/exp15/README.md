...
1. Get PDS table upload working over hardware SPI


## Current Sense Captures

This shows a successful call to `sl_wfx_init()` followed by `sl_wfx_deinit()`.
They reset the WF200, load the firmware, boot it, load the PDS table, then
shut down the WF200. The Saleae measures 0mV from the INA190 for the shutdown
mode, which indicates current draw somewhere between 0µA and 52µA (one LSB).

![capture_reset_firmware_boot_pds.png](capture_reset_firmware_boot_pds.png)

![capture_reset_from_shutdown.png](capture_reset_from_shutdown.png)

![capture_boot_and_pds_zoom.png](capture_boot_and_pds_zoom.png)
...
To get the PDS header file, do this, starting from `exp15/`:
```
cd ../
